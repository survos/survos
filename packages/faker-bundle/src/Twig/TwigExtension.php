...
return [
        // Faker\Provider\Uuid
                    ...
                    new TwigFunction($this->prefix . 'uuid_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'uuid_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'uuid_randomKey', fn($array=array (
                    ...
                    new TwigFunction($this->prefix . 'uuid_shuffleArray', fn($array=array (
                    ...
                    new TwigFunction($this->prefix . 'useragent_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'useragent_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'useragent_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'useragent_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'useragent_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'text_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'text_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'text_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'text_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'text_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'phonenumber_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'phonenumber_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'phonenumber_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'phonenumber_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'phonenumber_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'person_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'person_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'person_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'person_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'person_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'payment_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'payment_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'payment_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'payment_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'payment_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'miscellaneous_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'miscellaneous_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'miscellaneous_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'miscellaneous_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'miscellaneous_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'medical_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'medical_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'medical_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'medical_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'medical_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'lorem_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'lorem_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'lorem_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'lorem_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'lorem_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'internet_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'internet_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'internet_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'internet_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'internet_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'image_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'image_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'image_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'image_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'image_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'htmllorem_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'htmllorem_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'htmllorem_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'htmllorem_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'htmllorem_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'file_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'file_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'file_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'file_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'file_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'datetime_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'datetime_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'datetime_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'datetime_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'datetime_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'company_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'company_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'company_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'company_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'company_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'color_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'color_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'color_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'color_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'color_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'biased_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'biased_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'biased_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'biased_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'biased_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'barcode_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'barcode_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'barcode_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'barcode_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'barcode_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
                    ...
                    new TwigFunction($this->prefix . 'address_randomElements', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
), $count=1, $allowDuplicates=false) => $this->generator->randomElements($array,$count,$allowDuplicates)),  // @returns array
                    new TwigFunction($this->prefix . 'address_randomElement', fn($array=array (
  0 => 'a',
  1 => 'b',
  2 => 'c',
)) => $this->generator->randomElement($array)),  // @returns 
                    new TwigFunction($this->prefix . 'address_randomKey', fn($array=array (
)) => $this->generator->randomKey($array)),  // @returns int|string|null
                    new TwigFunction($this->prefix . 'address_shuffle', fn($arg='') => $this->generator->shuffle($arg)),  // @returns array
                    new TwigFunction($this->prefix . 'address_shuffleArray', fn($array=array (
)) => $this->generator->shuffleArray($array)),  // @returns array
