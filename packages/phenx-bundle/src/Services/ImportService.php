...
//        $con = \Propel::getConnection(DomainPeer::DATABASE_NAME);
        ...
        while ($d = $reader->getRow()) {
            ...
            if (!isset($protocol) || ($protocol->getPhenxId() <> $protocol_id)) {
                ...
                $protocol = $this->entityManager->getRepository(PhenxProtocol::class)
            ...
            /** @var Variable $v */
                $v = $this->entityManager->getRepository(Variable::class)
                    ->findOneOrCreate([
    ...
    public function scrape($class, $limit)
    {
        ...
        switch ($class) {
            case 'Measure':
                $repo = $this->entityManager->getRepository(Measure::class);
                break;
            case 'Protocol':
                $repo = $this->entityManager->getRepository(PhenxProtocol::class);
                break;
            default:
