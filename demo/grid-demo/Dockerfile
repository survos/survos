...
#FROM joseluisq/php-fpm:${PHP_VERSION} AS app_php
...
RUN docker-php-source extract \
    ...
    && apk add --update --virtual libxslt1-dev \
# Install pgsql goodness
    && apk add postgresql-dev \
    && docker-php-ext-install pgsql pdo_pgsql xsl \
    && apk del postgresql-libs libsasl db \
# Installing pecl modules
	&& pecl install apcu xdebug gd apcu \
# Enable pecl modules
